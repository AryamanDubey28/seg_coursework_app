...
describe("Items", () => {
    ...
    it("can not be deleted by a user that is not their author", async() => {
        ...
        const itemID = "item_abc"
        const setupDoc = setupDB.collection("items").doc(itemID);
        await firebase.assertSucceeds(setupDoc.set({userId:theirId,name:"name_before",illustration:"not_an_url",is_available:false}))
    ...
    it("can not be added if field is not of the right type", async() => {
        const db = getFirestore(myAuth);
        const setupDoc = db.collection("items");
        await firebase.assertFails(setupDoc.add({userId:myId,name:"a_name",is_available:12,illustration:"not_an_url"}))
    })

    it("can not be added with an unexpected field", async() => {
        const db = getFirestore(myAuth);
        const setupDoc = db.collection("items");
        await firebase.assertFails(setupDoc.add({userId:myId,name:"a_name",is_available:false,illustration:"not_an_url",not_expected:"not_expected"}))
    })

    it("can be added if all fields are correct (with add method)", async() => {
        const db = getFirestore(myAuth);
        const setupDoc = db.collection("items");
        await firebase.assertSucceeds(setupDoc.add({userId:myId,name:"a_name",illustration:"not_an_url",is_available:false}))
    })
}) 