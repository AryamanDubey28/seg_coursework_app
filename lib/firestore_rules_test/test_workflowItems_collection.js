...
async function createItemForMe(itemId)
{
    ...
    await setupDoc.set({userId:myId,name:"name_before",illustration:"not_an_url",is_available:false})
}
...
describe("WorkflowItems", () => {
    ...
    it("can be deleted by their author", async() => {
        ...
        await firebase.assertSucceeds(setupWorkflowItem.set({userId:myId,name:"item_name",illustration:"not_an_url",rank:12,is_available:false}))

        const testDoc = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
    ...
    it("can not be deleted by a user that is not their author", async() => {
        ...
        const setupWorkflowItem = setupDB.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertSucceeds(setupWorkflowItem.set({userId:theirId,name:"item_name",illustration:"not_an_url",rank:12,is_available:false}))
    ...
    it("can be edited by their author", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await setupWorkflowItem.set({userId:myId,name:"item_name",illustration:"not_an_url",rank:12,is_available:false})
    ...
    it("can not be edited by a user that is not their author", async() => {
        ...
        await setupWorkflowItem.set({userId:theirId,name:"item_name",illustration:"not_an_url",rank:12,is_available:false})
    ...
    it("can not create a workflowItem if not logged in", async() => {
        ...
        const created = await createWorkflowForThem(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:theirId,name:"item_name",illustration:"not_an_url",rank:12,is_available:false}))
    })
    ...
    it("can not create a workflowItem for a user that is not us", async() => {
        ...
        const created = await createWorkflowForThem(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,name:"item_name",illustration:"not_an_url",rank:12,is_available:false}))
    })
    ...
    it("has to be created with a name field", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,illustration:"not_an_url",rank:12,is_available:false}))
    })
    ...
    it("has to be created with an illustration field", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,name:"a_name",rank:12,is_available:false}))
    })
    ...
    it("has to be created with a rank field", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,illustration:"not_an_url",illustration:"not_an_url",is_available:false}))
    })
    ...
    it("has to be created with a userId field", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({rank:12,illustration:"not_an_url",illustration:"not_an_url",is_available:false}))
    })
    ...
    it("field name has to be a string", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,name:12,illustration:"not_an_url",rank:12,is_available:false}))
    })
    ...
    it("field illustration has to be a string", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,name:"a_name",illustration:12,rank:12,is_available:false}))
    })
    ...
    it("field rank has to be a number", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:"not valid",is_available:false}))
    })
    ...
    it("field rank can not be negative", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:-12,is_available:false}))
    })
    ...
    it("field rank can be zero", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertSucceeds(setupWorkflowItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:0,is_available:false}))
    })
    ...
    it("field rank can be positive", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertSucceeds(setupWorkflowItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:12,is_available:false}))
    })
    ...
    it("can not be created for an item that does not exist (using update() method)", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+ workflow_id +"/items").doc("item_xyz");
        await firebase.assertFails(setupWorkflowItem.update({userId:myId,name:"a_name",illustration:"not_an_url",rank:12,is_available:false}))
    })
    ...
    it("has to be created with an is_available field", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:0}))
    })
    ...
    it("is_available field has to be a boolean", async() => {
        ...
        const created = await createWorkflowForMe(workflow_id)
        const setupWorkflowItem = db.collection("workflowItems/"+workflow_id+"/items").doc(itemID);
        await firebase.assertFails(setupWorkflowItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:0,is_available:12}))
    })
}) 
