...
async function createWorkflowForThem(workflowID)
{
    const db = getFirestore(theirAuth);
    const setupDoc = db.collection("workflows").doc(workflowID);
    await firebase.assertSucceeds(setupDoc.set({userId:theirId,title:"a_title"}))
}
...
describe("WorkflowItems", () => {
    ...
    it("can not be deleted by a user that is not their author", async() => {
        ...
        await firebase.assertSucceeds(setupWorkflowItem.set({userId:theirId,name:"item_name",illustration:"not_an_url",rank:12}))
