...
describe("Workflows", () => {
    ...
    it("can not be deleted by a user that is not their author", async() => {
        ...
        await firebase.assertSucceeds(setupDoc.set({userId:theirId,title:"title_before",illustration:"not_an_url"}))
    ...
    it("can not be added if field is not of the right type", async() => {
        ...
        await firebase.assertFails(setupDoc.add({userId:myId,title:"a_name",illustration:12}))
    })

    it("can not be added with an unexpected field", async() => {
        const db = getFirestore(myAuth);
        const setupDoc = db.collection("workflows");
        await firebase.assertFails(setupDoc.add({userId:myId,title:"a_name",illustration:"URL",unexpected:"not_allowed"}))
    })

    it("can be added if all fields are correct", async() => {
        const db = getFirestore(myAuth);
        const setupDoc = db.collection("workflows");
        await firebase.assertSucceeds(setupDoc.add({userId:myId,title:"a_name",illustration:"URL"}))
    })
}) 
