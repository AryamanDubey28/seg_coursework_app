...
async function createCategoryForMe(categoryId)
{
    ...
    await setupDoc.set({userId:myId,title:"category_title",illustration:"not_an_url",rank:2,is_available:true})
}
...
async function createItemForMe(itemId)
{
    ...
    await setupDoc.set({userId:myId,name:"name_before",illustration:"not_an_url",is_available:true})
}
...
async function createCategoryForThem(categoryId)
{
    ...
    const setupDoc = db.collection("categories").doc(categoryId);
    await setupDoc.set({userId:theirId,title:"category_title",illustration:"not_an_url",rank:2,is_available:true})
}
...
async function createItemForThem(itemId)
{
    const db = getFirestore(theirAuth);
    const setupDoc = db.collection("items").doc(itemId);
    await setupDoc.set({userId:theirId,name:"name_before",illustration:"not_an_url",is_available:true})
}
...
describe("CategoryItems", () => {
    ...
    it("can be created by their author", async() => {
        ...
        await firebase.assertSucceeds(setupCategoryItem.set({userId:myId,name:"item_name",illustration:"not_an_url",rank:12,is_available:true}))
    })
    ...
    it("can be deleted by their author", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await setupCategoryItem.set({userId:myId,name:"item_name",illustration:"not_an_url",rank:12,is_available:true})

        const testDoc = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
    ...
    it("can not be deleted by a user that is not their author", async() => {
        ...
        const setupCategoryItem = setupDB.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await setupCategoryItem.set({userId:theirId,name:"item_name",illustration:"not_an_url",rank:12,is_available:true})
    ...
    it("can not create a categoryItem if not logged in", async() => {
        ...
        const created_category = createCategoryForThem(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:theirId,name:"item_name",illustration:"not_an_url",rank:12,is_available:true}))
    })
    ...
    it("can not create a categoryItem for a user that is not us", async() => {
        ...
        const created_category = createCategoryForThem(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:"item_name",illustration:"not_an_url",rank:12,is_available:true}))
    })
    ...
    it("has to be created with a name field", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,illustration:"not_an_url",rank:12,is_available:true}))
    })
    ...
    it("has to be created with an illustration field", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:"a_name",rank:12,is_available:true}))
    })
    ...
    it("has to be created with a rank field", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:"a_name",illustration:"this_is_not_an_url",is_available:true}))
    })
    ...
    it("has to be created with a userId field", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({rank:12,name:"a_name",illustration:"this_is_not_an_url",is_available:true}))
    })
    ...
    it("field name has to be a string", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:12,illustration:"not_an_url",rank:12,is_available:true}))
    })
    ...
    it("field illustration has to be a string", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:"a_name",illustration:12,rank:12,is_available:true}))
    })
    ...
    it("field rank has to be a number", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:"not valid",is_available:true}))
    })
    ...
    it("field rank can not be negative", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:-4,is_available:true}))
    })
    ...
    it("field rank can be zero", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertSucceeds(setupCategoryItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:0,is_available:true}))
    })
    ...
    it("field rank can be positive", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertSucceeds(setupCategoryItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:1,is_available:true}))
    })
    ...
    it("can not be created for a category that does not exist", async() => {
        ...
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:1,is_available:true}))
    })
    ...
    it("can not be created for an item that does not exist (using update() method)", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+ categoryId +"/items").doc("item_xyz");
        await firebase.assertFails(setupCategoryItem.update({userId:myId,name:"a_name",illustration:"not_an_url",rank:1,is_available:true}))
    })
    ...
    it("has to be created with an is_available field", async() => {
        ...
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:0}))
    })

    it("is_available field has to be a boolean", async() => {
        const db = getFirestore(myAuth);
        const itemID = "item_abc"
        const created_item = createItemForMe(itemID)
        const categoryId = "category_abc"
        const created_category = createCategoryForMe(categoryId)
        const setupCategoryItem = db.collection("categoryItems/"+categoryId+"/items").doc(itemID);
        await firebase.assertFails(setupCategoryItem.set({userId:myId,name:"a_name",illustration:"not_an_url",rank:0,is_available:12}))
    })
}) 
