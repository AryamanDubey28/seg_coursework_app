...
describe("Categories", () => {
    ...
    it("can not be deleted by a user that is not their author", async() => {
        ...
        await firebase.assertSucceeds(setupDoc.set({userId:theirId,title:"category_title",illustration:"not_an_url",rank:2}))
    ...
    it("can not be added if field is not of the right type", async() => {
        ...
        await firebase.assertFails(setupDoc.add({userId:myId,title:"a_title",illustration:12,rank:0}))
    })

    it("can not be added with an unexpected field", async() => {
        const db = getFirestore(myAuth);
        const setupDoc = db.collection("categories");
        await firebase.assertFails(setupDoc.add({userId:myId,title:"a_title",illustration:"not_an_url",rank:0,unexpected:12}))
    })

    it("can be added if all fields are correct", async() => {
        const db = getFirestore(myAuth);
        const setupDoc = db.collection("categories");
        await firebase.assertSucceeds(setupDoc.add({userId:myId,title:"a_title",illustration:"not_an_url",rank:0}))
    })
}) 
