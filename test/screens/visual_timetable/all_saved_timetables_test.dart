...
void main() {
  ...
  Future<void> _createTimetable() async {
    ...
    await firebaseFunctions.saveWorkflowToFirestore(timetable: Timetable(listOfImages: [toastItem, testItems[1]]));
  }
  ...
  testWidgets('A dialog that contains the timetable is shown when the timetable is tapped.', 
    (WidgetTester tester) async {
      mockNetworkImagesFor(() async {
  ...
  testWidgets('Tapping a timetable item crosses it out.', 
    ...
    expect(
      ...
      await tester.tap(find.byKey(const ValueKey("timetableDialogImage0")), warnIfMissed: false);
      await tester.pump();
